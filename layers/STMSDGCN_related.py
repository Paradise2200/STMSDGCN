...
def do_patching(z, stride, patch_len):
    ...
    if z.shape[-1] < patch_len:
        z = nn.functional.pad(z, (0, patch_len - z.shape[-1])).contiguous()
    # real doing patching
    z = z.unfold(dimension=-1, size=patch_len, step=stride).contiguous()  # z: [bs x nvars x patch_num x patch_len]
    return z
...
class TTransformer(nn.Module):
    def __init__(self, embed_size, heads, time_num, dropout, stride, patch_len, **kwargs):
        ...
        self.temporal_embedding = nn.Embedding(int(time_num),
                                               embed_size * patch_len)  # temporal embedding选用nn.Embedding
    ...
    def forward(self, x):
        ...
        out = U_t + x + M_t  # 残差连接
        out = out.reshape(out.size(0), out.size(1), -1, out.size(2) * self.patch_len).contiguous()
...
class TSelfattention(nn.Module):
    ...
    def forward(self, value, key, query):
        ...
        keys = key.reshape(B, N, T, self.heads, self.per_dim).contiguous()
        queries = query.reshape(B, N, T, self.heads, self.per_dim).contiguous()
        values = value.reshape(B, N, T, self.heads, self.per_dim).contiguous()
        ...
        attnscore = torch.einsum("bnqhd, bnkhd->bnqkh", (queries, keys)).contiguous()  # [B,N, T, T, heads]
        attention = torch.softmax(attnscore / (self.embed_size ** (1 / 2)), dim=2)

        out = torch.einsum("bnqkh, bnkhd->bnqhd", (attention, values)).contiguous()  # [B, N, T, heads, per_dim]
        out = out.reshape(B, N, T, self.embed_size).contiguous()
        out = self.fc(out)
...
class nconv(nn.Module):
    ...
    def forward(self, x, A):
        A = A.transpose(-1, -2).to(x.device).contiguous()
...
class GCNPool(nn.Module):
    def __init__(self, c_in, c_out, num_nodes, tem_size,
        ...
        self.f_gcn = F_GCN(args, c_in, c_out, num_nodes, tem_size,
                           Kt, dropout, pool_nodes, support_len, order)
        ...
        self.TAT = TTransformer(c_out, heads=args.heads, time_num=time_num, dropout=dropout, stride=args.stride,
                                patch_len=args.patch_len, out_len=out_len)
        self.c_out = c_out
...
class F_GCN(nn.Module):
    def __init__(self, args, c_in, c_out, num_nodes, tem_size,
                 Kt, dropout, pool_nodes, support_len=3, order=2):
        super(F_GCN, self).__init__()
        self.args = args
        self.c_out = c_out
        self.multigcn = nn.ModuleList([mutil_gcn_time(c_out, 2 * c_out, dropout=dropout, support_len=support_len, Kt=1,
                                                      nhid=args.d_model, num_gcn=args.gcn) for wi in
                                       range(args.num_gcn)])

        self.mlp = torch.nn.Conv2d(c_in, c_out, kernel_size=(1, Kt), padding=(0, 0), stride=(1, 1), bias=True)

        if args.connect == 'residual':
            self.fc = nn.ModuleList(
                [nn.Conv2d(c_in, c_out, kernel_size=(1, 1), bias=True) for i in range(args.num_gcn)])
        elif args.connect == 'skip':
            self.fc = nn.ModuleList([nn.Conv2d(int(c_in * i), c_in, kernel_size=(1, 1), bias=True) for i in
                                     range(2, args.num_gcn + 2)])
        elif args.connect == 'one_residual':
    ...
    def forward(self, x, support):
        skip_list = []
        if self.args.connect == 'skip':
            skip_list.append(x)

        for i in range(self.args.num_gcn):
            if self.args.connect != 'skip':
                skip = x.clone()

            x = self.multigcn[i](x, support)
            x1, x2 = torch.split(x, [self.c_out, self.c_out], 1)
            x = torch.tanh(x1) * torch.sigmoid(x2)
            ...
            if self.args.connect == 'residual':
                fc = self.fc[i]
                x = x + fc(skip)

            elif self.args.connect == 'skip':
                x = torch.concat(skip_list, dim=1)
                fc = self.fc[i]
                x = fc(x)

            elif self.args.connect == 'one_residual':
                fc = self.fc
                x = x + fc(skip)

            elif self.args.connect == 'add_residual':
                x = x + skip
...
class GraphConvolution_(nn.Module):
    ...
    def forward(self, x, adj):
        support_q = torch.sigmoid(torch.einsum('bcnl,kcd->kbdnl', x, self.weight_q).contiguous())
        support_v = torch.einsum('bcnl,kcd->kbdnl', x, self.weight_v).contiguous()
        support = torch.sum(torch.multiply(support_q, support_v), dim=0)
        output = torch.einsum('nk,bcnl->bckl', adj.cuda(), support.cuda()).contiguous()
        if self.bias is not None:
